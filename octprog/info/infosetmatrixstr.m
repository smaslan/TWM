...
%   Optimized: no
        ...
        if (~ischar(infostr) || ~ischar(key))
                error('infosetmatrixstr: infostr and key must be strings')
        end
        if (~iscell(val) || ~all(cellfun(@ischar, val)))
                error('infosetmatrixstr: val must be a string cell matrix')
        end
        if (~iscell(scell))
                error('infosetmatrixstr: scell must be a cell')
        end
        if (~all(cellfun(@ischar, scell)))
                error('infosetmatrixstr: scell must be a cell of strings')
        end
