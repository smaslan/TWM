...
% Part of QWTB. Test script for algorithm TWM-TIAZ.
%
...
% See also qwtb
    ...
    % supply voltage [V]:
    Uref = 0.5;
    % transimpedance value [Ohm]:    
    Ztia = 5000.0*exp(10e-6*j);
    % DUT impedance [Ohm]:    
    Zdut  = 10e3*exp(-50e-6*j);
    % DUT voltage transducer ratio [Vin/Vout]
    r_dut = 1667.0; 
    ...
    chns{id}.type = 'shunt';
    % harmonic amplitudes:
    chns{id}.A = 2^0.5*[abs(U_tia)];
    % harmonic phases:
    chns{id}.ph = [angle(U_tia)];
    ...
    % generate TIA output ADC:
    din.i_adc_aper_corr.v = 1;
    din.i_adc_gain_f.v = [];
    din.i_adc_gain_a.v = [];    
    din.i_adc_gain.v   = linrand(0.9,1.1)/abs(Ztia);
    din.i_adc_gain.u   = 0;
    din.i_adc_phi_f.v = [];
    din.i_adc_phi_a.v = [];    
    ...
    % generate TIA output transducer:
    din.i_tr_gain_f.v = [];
    din.i_tr_gain_a.v = [];    
    din.i_tr_gain.v   = 1.0;
    din.i_tr_gain.u   = 0;
...
%     hold off;
    ...
    dout = qwtb('TWM-TIAZ',datain,calcset);
