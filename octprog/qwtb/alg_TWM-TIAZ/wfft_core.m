function dataout = wfft_core(datain, cfg, tab, calcset, fs)
...
% The script is distributed under MIT license, https://opensource.org/licenses/MIT.                
    ...
    if ~exist('is_wfft_local','var')
        % check if we can run WFFT using local function instead of QWTB
        persistent is_wfft_local;        
        try                
            if isfield(datain,'window')
                window = datain.window.v;
            else
                window = 'rect';    
            end        
            ampphspectrum(zeros(100,1), 1.0, 0, 0, window, [], 0);
            % seems all functions are available, so use local mode
            is_wfft_local = 1;
        catch
            % looks like some function is missing, so run via QWTB
            is_wfft_local = 0;
        end
    end
    ...
    if ~isfield(datain,'f_nom')
        ...
        if is_psfe_local
            ...
            datain.f_nom.v = PSFE(y_psfe, 1/fs);        
        else
            % call PSFE via QWTB (slower, but cleaner):
            din = struct();
            din.fs.v = fs;        
            cset = calcset;
            cset.verbose = 0;
    ...
    if is_wfft_local
        % call wfft directly (faster):        
        for r = 1:size(datain.y.v,2)
            [fh, A(:,r), ph(:,r), w] = ampphspectrum(datain.y.v(:,r), fs, 0, 0, din.window.v, [], 0);
        end
        fh = fh(:);
    ...
    if cfg.y_is_diff
        ...
        if is_wfft_local
            % call wfft directly (faster):        
            for r = 1:size(datain.y_lo.v,2)
                [fnull, A_lo(:,r), ph_lo(:,r), w] = ampphspectrum(datain.y_lo.v(:,r), fs, 0, 0, din.window.v, [], 0);
            end  
        else
            % call wfft via QWTB (slower, but cleaner):    
