function [res] = proc_wfft_test(par)
% TWM-WFFT validation - execution of test runs for given test setup.
    ...
    if strcmpi(mc_setup.method,'for')
        ...
        for r = 1:par.val.max_count            
            try
                runres{end+1,1} = proc_wfft_test_run(jobs{r});
            end
        ...
        runres = runmulticore(mc_setup.method, @proc_wfft_test_run, jobs, mc_setup.cores, mc_setup.share_fld, 2, mc_setup);
    end
